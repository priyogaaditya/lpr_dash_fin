    ...
    html.Div([
        html.Div([
            ...
            dcc.Input(id="vehicle_type_year",
                    placeholder='Enter selected year (YYYY)',
                    type='text',
                    persistence = False,
        ...
        html.Div([
            ...
            dcc.Input(id="vehicle_type_month_start",
                    placeholder='Enter starting month (MM)',
                    type='text',
                    persistence = False,
                    style={'width': '400px','padding':'10px', 'border-radius':'10px'}
            )
        ...
        html.Div([
            ...
            dcc.Input(id="vehicle_type_month_stop",
                    placeholder='Enter stop month (MM)',
                    type='text',
                    persistence = False,
                    style={'width': '400px','padding':'10px', 'border-radius':'10px'}
            )
        ...
        html.Div([
            ...
            dcc.Input(id="vehicle_type_date_start",
                    placeholder='Enter starting date (DD)',
                    type='text',
                    persistence = False,
                    style={'width': '400px','padding':'10px', 'border-radius':'10px'}
            )
        ...
        html.Div([
            ...
            dcc.Input(id="vehicle_type_date_stop",
                    placeholder='Enter stop date (DD)',
                    type='text',
                    persistence = False,
                    style={'width': '400px','padding':'10px', 'border-radius':'10px'}
            )
        ...
        html.Div([
            ...
            dcc.Input(id="vehicle_type_hour_start",
                    placeholder='Enter starting hour (HH)',
                    type='text',
                    persistence = False,
                    style={'width': '400px','padding':'10px', 'border-radius':'10px'}
            )
        ...
        html.Div([
            ...
            dcc.Input(id="vehicle_type_hour_stop",
                    placeholder='Enter stop hour (HH)',
                    type='text',
                    persistence = False,
                    style={'width': '400px','padding':'10px', 'border-radius':'10px'}
            )
        ...
        html.Button('Submit', id='vehicle_type_submit_button', n_clicks=0, disabled=False, 
                    style = button_summit)        
    ], style=content_flex_style),
    html.Div([
        html.Div([
            html.H2("Date", style={'margin':'0 auto','font-size': '15px', 'padding-bottom':'5px'}),
            html.Div([
                html.P(''+str(datetime.today().strftime("%d %B %Y, %H:%M")), style={'margin':'0 auto'}),
            ])
        ], style={'background-color':'white', 'padding':'10px', 'color':'black', 'border-radius':'10px', 'width':'40%'}),
        html.Div([
            dcc.Dropdown(
        ...
        html.Div([
            html.H2("Graph", style={'margin':'0 auto', 'color':'black', 'margin-bottom':'20px', 'font-size': '15px'}),
            html.Div(id='vehicle-type-output-container')
        ], style=graph_style)        
    ], style={'width':'50%'})
], style=content_tab_style)
...
app.layout = html.Div([
    html.Div([
        html.Img(src='https://i.ibb.co/9wC7KNn/LPR-black.png', style = {'height': '30px', 'width': 'auto'}),
        html.Img(src='https://i.ibb.co/RG8gcwc/x-camp.jpg', style = {'height': '30px', 'width': 'auto'})
    ], style = {'background': 'white', 'box-shadow': '0 14px 28px lightgrey', 'text-align':'center', 'padding':'20px', 'margin': '-10px'}),    
    html.H1('License Plate Recognition', style = {'text-align': 'start', 'padding':'20px 40px 0', 'font-family': "'Poppins', sans-serif"}),
    html.Div([
        dcc.Tabs([
...
def update_ganjil_genap(n_click, dropdown_value_option, stored_inputs):
    ...
    if stored_inputs is not None:
        ...
        engine = create_engine('mysql+pymysql://' + DBconfig.user + ':' + DBconfig.pwd + '@' + DBconfig.host + ':' + str(DBconfig.port) + '/' + DBconfig.db_name , echo=False)
        query = f"SELECT * FROM lpr_table where timestamp >= '{datetime_start}' AND timestamp <= '{datetime_stop}'"
        lpr_data = pd.read_sql(query, con=engine)
